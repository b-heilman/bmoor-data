...
describe('bmoor-data.compiler.calculation', function () {
	...
	describe('- Compiler', function () {
		describe('::tokenize', function () {
			describe('single parsing', function () {
				it('should work for blocks', function () {
					expect(
						JSON.parse(
							JSON.stringify(compiler.tokenize('(like, this)')[0].tokens)
						)
					).to.deep.equal([
						{
							type: 'block',
				...
				it('should work for operations', function () {
					expect(
						JSON.parse(JSON.stringify(compiler.tokenize('&&')[0].tokens))
					).to.deep.equal([
						{
							type: 'operation',
							metadata: null,
				...
				it('should work for number (int)', function () {
					expect(
						JSON.parse(JSON.stringify(compiler.tokenize('123')[0].tokens))
					).to.deep.equal([
						{
							type: 'constant',
							metadata: {
				...
				it('should work for number (float)', function () {
					expect(
						JSON.parse(JSON.stringify(compiler.tokenize('123.23')[0].tokens))
					).to.deep.equal([
						{
							type: 'constant',
							metadata: {
				...
				it('should work for string (")', function () {
					expect(
						JSON.parse(
							JSON.stringify(compiler.tokenize('"foo-bar it\'s"')[0].tokens)
						)
					).to.deep.equal([
						{
							type: 'constant',
				...
				it('should work under a multi statement', function () {
					...
					expect(JSON.parse(JSON.stringify(sets[0].tokens))).to.deep.equal([
						{
							type: 'method',
							metadata: {
								name: 'fooBar',
					...
					expect(JSON.parse(JSON.stringify(sets[1].tokens))).to.deep.equal([
						{
							type: 'method',
							metadata: {
								name: 'base'
			...
			describe('mixed parsing', function () {
				...
				it('should parse a method', function () {
					expect(
						JSON.parse(
							JSON.stringify(
								compiler.tokenize('drumpf(like, 123)boo blah(hello & world)')[0]
									.tokens
							)
						)
					).to.deep.equal([
				...
				it('should parse operations with this', function () {
					expect(
						JSON.parse(
							JSON.stringify(
								compiler.tokenize('$foo.bar && $hello.world || 123 > $eins')[0]
									.tokens
							)
						)
					).to.deep.equal([
		...
		describe('::compile', function () {
			it('should hopefully work - 1', function () {
				expect(
					JSON.parse(
						JSON.stringify(
							compiler.compile(compiler.tokenize('$a+$b*$c-$d/$e')[0].tokens)
								.postfix
						)
					)
				).to.deep.equal([
			...
			it('should hopefully work - 2', function () {
				expect(
					JSON.parse(
						JSON.stringify(
							compiler.compile(compiler.tokenize('$a*$b+$c*$d')[0].tokens)
								.postfix
						)
					)
				).to.deep.equal([
		...
		describe('::prepare', function () {
			...
			it('should allow string comparison', function () {
				expect(compiler.prepare('$a == "hello"')({a: 'hello'})).to.deep.equal([
					true
				]);

				expect(compiler.prepare('$a == "world"')({a: 'hello'})).to.deep.equal([
					false
				]);
			});

			it('should allow a comparison to null', function () {
				expect(compiler.prepare('$a == null')({a: null})).to.deep.equal([true]);

				expect(compiler.prepare('$a == null')({a: 'hello'})).to.deep.equal([
					false
				]);
			});

			it('should allow a comparison to undefined', function () {
				expect(compiler.prepare('$a == undefined')({})).to.deep.equal([true]);

				expect(compiler.prepare('$a == undefined')({a: 'hello'})).to.deep.equal(
					[false]
				);
			});

			it('should work as a multipart statement', function () {
				expect(compiler.prepare('$a,$b')({a: 1, b: 2})).to.deep.equal([1, 2]);
			});

			it('should work as a multipart statement and complex variables', function () {
				expect(
					compiler.prepare('${a-b},${c-d}')({'a-b': 1, 'c-d': 2})
				).to.deep.equal([1, 2]);
			});

			it('should work with a method in the statement', function () {
				expect(compiler.prepare('sum($a,$b)')({a: 1, b: 2})).to.deep.equal([3]);
			});

			it('should work with a methods inside methods', function () {
				expect(
					compiler.prepare('join("|",4,$c,sum($a,$b))')({a: 1, b: 2, c: 3})
				).to.deep.equal(['4|3|3']);
			});
		});
	});
	...
	describe(' - Normalized', function () {
		...
		it('should inflate a singular expression correctly', function () {
			...
			expect(eb.toJSON()).to.deep.equal([
				{type: 'value', method: 'getter'},
