...
describe('bmoor-data::/model/Mapper', function () {
	it('should properly instantiate', function () {
		...
		expect(table1.joins).to.deep.equal([
			{
				name: 'table-2',
				local: 'id',
				remote: 'eins',
		...
		expect(table1.hash).to.deep.equal({
			'table-2': {
				name: 'table-2',
				local: 'id',
				remote: 'eins',
		...
		expect(table2.joins).to.deep.equal([
			{
				name: 'table-1',
				local: 'eins',
				remote: 'id',
		...
		expect(table2.hash).to.deep.equal({
			'table-1': {
				name: 'table-1',
				local: 'eins',
				remote: 'id',
		...
		expect(table3.joins).to.deep.equal([
			{
				name: 'table-2',
				local: 'zwei',
				remote: 'id',
		...
		expect(table3.hash).to.deep.equal({
			'table-2': {
				name: 'table-2',
				local: 'zwei',
				remote: 'id',
		...
		expect(table4.joins).to.deep.equal([
			{
				name: 'table-3',
				local: 'drei',
				remote: 'id',
		...
		expect(table4.hash).to.deep.equal({
			'table-3': {
				name: 'table-3',
				local: 'drei',
				remote: 'id',
	...
	describe('::getRelationships', function () {
		...
		it('pull in a singlar value', function () {
			...
			expect(mapper.getRelationships(model1)).to.deep.equal([
				{
					name: 'test-2',
					local: 'zwei',
					remote: 'id',
			...
			expect(mapper.getRelationships(model3)).to.deep.equal([
				{
					name: 'test-2',
					local: 'zwei',
					remote: 'id',
			...
			expect(mapper.getRelationships(model2)).to.deep.equal([
				{
					name: 'test-1',
					local: 'id',
					remote: 'zwei',
	...
	describe('::getRelationship', function () {
		...
		it('pull in a singlar value', function () {
			...
			expect(mapper.getRelationship(model1, 'test-2')).to.deep.equal({
				name: 'test-2',
				local: 'zwei',
				remote: 'id',
				metadata: {direction: 'outgoing'}
			...
			expect(mapper.getRelationship(model1, 'test-3')).to.deep.equal({
				name: 'test-3',
				local: 'fier',
				remote: 'id',
				metadata: {direction: 'outgoing'}
			...
			expect(mapper.getRelationship(model1, 'test-3', 'drei')).to.deep.equal({
				name: 'test-3',
				local: 'drei',
				remote: 'id',
				metadata: {direction: 'outgoing'}
			...
			expect(mapper.getRelationship(model1, 'test-3', 'fier')).to.deep.equal({
				name: 'test-3',
				local: 'fier',
				remote: 'id',
				metadata: {direction: 'outgoing'}
			...
			expect(mapper.getRelationship(model1, 'test-4')).to.equal(null);
		});
	});
