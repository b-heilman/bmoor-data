...
async function install(action, service, master, mapper, ctx) {
	...
	if (action.$type === 'read') {
		...
		if (!datum) {
			throw create(
				`unable to read expected datum of type ${service.model.name}`,
				{
					status: 406,
		...
		if (!current) {
			throw create(
				`unable to update expected datum of type ${service.model.name}`,
				{
					status: 406,
	...
	mapper.getByDirection(service.model.name, 'incoming').forEach((link) => {
...
async function inflate(service, query, mapper, registry, ctx) {
	const known = {};
	const looking = {};
	const toProcess = query.keys.map((key) => ({
		ref: null,
		query: key,
		service
	}));
	const joinModels = Object.keys(query.join || []).reduce((agg, model) => {
		const tables = query.join[model];

		agg[model] = tables.reduce((agg, table) => {
			agg[table] = true;
	...
	const ensureModels = (query.ensure || []).reduce(
		// read-create, no update
		(agg, table) => {
			agg[table] = true;
	...
	do {
		...
		datums.forEach((datum) => {
			...
			if (!stubbed) {
				mapper
					.getByDirection(service.model.name, 'outgoing')
					.forEach((link) => {
						// jshint ignore:line
				...
				if (lookup) {
					mapper
						.getByDirection(service.model.name, 'incoming')
						.forEach((link) => {
							if (lookup[link.name]) {
								toProcess.push({
									ref: ref,
...
async function clear(master, mapper, registry, ctx) {
	...
	return order.reduce((prom, serviceName) => {
		...
		return master[serviceName].reduce(
			(agg, datum) => agg.then(() => service.delete(datum, ctx)),
			prom
		);
	}, Promise.resolve(true));
}
