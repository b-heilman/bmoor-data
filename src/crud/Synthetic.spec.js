...
describe('bmoor-data::crud/Synthetic', function () {
	...
	describe('::deflate', function () {
		...
		it('should properly process a basic set of models', function (done) {
			stubs.class1 = sinon.stub(class1, 'create').resolves({id: 123});

			stubs.class2 = sinon.stub(class2, 'create').resolves({id: 456});

			stubs.class3 = sinon.stub(class3, 'create').resolves({id: 789});

			stubs.class4 = sinon.stub(class4, 'create').resolves({id: 0});
			...
			deflate(
				...
				.then(() => {
					expect(stubs.class1.getCall(0).args[0]).to.deep.equal({
						$ref: 'foo-1'
					});

					expect(stubs.class2.getCall(0).args[0]).to.deep.equal({
						$ref: 'bar-2',
						class1Id: 123
					});

					expect(stubs.class3.getCall(0).args[0]).to.deep.equal({
						class2Id: 456
					});
		...
		it('should properly process a read (via read) with a set of models', function (done) {
			stubs.class1 = sinon.stub(class1, 'read').resolves({id: 123});
			...
			deflate(
				...
				.then(() => {
					expect(stubs.class1.getCall(0).args[0]).to.equal('one');
		...
		it('should properly process a read (via query) with a set of models', function (done) {
			stubs.class1 = sinon.stub(class1, 'query').resolves([{id: 123}]);
		...
		it('should properly process an update (via read) with a set of models', function (done) {
			stubs.class1Read = sinon.stub(class1, 'read').resolves({id: 123});

			stubs.class1Update = sinon.stub(class1, 'update').resolves({id: 123});
			...
			deflate(
				...
				.then(() => {
					expect(stubs.class1Read.getCall(0).args[0]).to.equal('one');

					expect(stubs.class1Update.getCall(0).args[0]).to.equal(123);

					expect(stubs.class1Update.getCall(0).args[1]).to.deep.equal({
						$type: 'update',
						$ref: 'foo-1',
						id: 'one'
					});
		...
		it('should properly process an update (via query) with a set of models', function (done) {
			stubs.class1Read = sinon.stub(class1, 'query').resolves([{id: 123}]);
			...
			deflate(
				...
				.then(() => {
					expect(stubs.class1Read.getCall(0).args[0]).to.deep.equal({
						info: 'one'
					});
	...
	describe('::inflate', function () {
		...
		describe('class1', function () {
			it('should properly with a single key', function (done) {
				stubs.class1 = sinon.stub(class1, 'read').resolves({
					id: 123,
					hello: 'world'
				});
				...
				inflate('class-1', {keys: ['key-1']}, mapper, serviceRegistry, {})
					.then((instructions) => {
						expect(stubs.class1.getCall(0).args[0]).to.equal('key-1');
			...
			it('should properly with multiple keys', function (done) {
				...
				inflate(
					...
					.then((instructions) => {
						...
						expect(stubs.class1.getCall(1).args[0]).to.equal('key-2');
		...
		describe('class3', function () {
			it('should properly with a single key', function (done) {
				stubs.class1 = sinon.stub(class1, 'query').resolves([
					{
						n: 1,
						id: 123
					}
				...
				stubs.class2 = sinon.stub(class2, 'query').resolves([
					{
						n: 2,
						id: 123,
						class1Id: 'key-1-1'
				...
				stubs.class3 = sinon.stub(class3, 'read').resolves({
					n: 3,
					id: 123,
					class2Id: 'key-2-1'
				});
				...
				inflate('class-3', {keys: ['key-1']}, mapper, serviceRegistry, {})
					.then((instructions) => {
						...
						expect(stubs.class3.getCall(0).args[0]).to.equal('key-1');
			...
			it('should properly with multiple keys', function (done) {
				...
				inflate(
					...
					.then((instructions) => {
						...
						expect(stubs.class2.getCall(1).args[0]).to.deep.equal({
							id: 'key-2-2'
						});

						expect(stubs.class3.getCall(0).args[0]).to.equal('key-1');

						expect(stubs.class3.getCall(1).args[0]).to.equal('key-2');

						expect(stubs.class3.getCall(2).args[0]).to.equal('key-3');
		...
		describe('class4', function () {
			it('should properly with a single key', function (done) {
				...
				stubs.class4 = sinon.stub(class4, 'read').resolves({
					n: 2,
					id: 123,
					class1Id: 'key-1-1'
				});
				...
				inflate('class-4', {keys: ['key-1']}, mapper, serviceRegistry, {})
					.then((instructions) => {
						...
						expect(stubs.class4.getCall(0).args[0]).to.equal('key-1');
		...
		describe('class3 - class4 joined into', function () {
			it('should properly with a single key', function (done) {
				...
				stubs.class4 = sinon.stub(class4, 'query').resolves([
					{
						n: 4,
						id: 456,
						class1Id: 123
				...
				inflate(
					...
					.then((instructions) => {
						expect(stubs.class1.getCall(0).args[0]).to.deep.equal({id: 123});

						expect(stubs.class2.getCall(0).args[0]).to.deep.equal({id: 234});

						expect(stubs.class3.getCall(0).args[0]).to.equal(345);

						expect(stubs.class4.getCall(0).args[0]).to.deep.equal({
							class1Id: 123
						});
			...
			it('should properly allow the stubbing of a table', function (done) {
				...
				inflate(
					'class-3',
					{keys: [345], stub: ['class-2']},
					mapper,
					serviceRegistry,
					{}
				)
					.then((instructions) => {
						expect(stubs.class1.getCall(0)).to.equal(null);

						expect(stubs.class2.getCall(0).args[0]).to.deep.equal({id: 234});

						expect(stubs.class3.getCall(0).args[0]).to.equal(345);

						expect(stubs.class4.getCall(0)).to.equal(null);
	...
	describe('pivot time', function () {
		...
		describe('::inflate', function () {
			it('should work from class 1', function (done) {
				...
				stubs.class3 = sinon.stub(class3, 'query').resolves([
					{
						n: 3,
						id: 345
					}
				...
				inflate(
					...
					.then((instructions) => {
						expect(stubs.class1.getCall(0).args[0]).to.equal(123);

						expect(stubs.class2.getCall(0).args[0]).to.deep.equal({
							class1Id: 123
						});

						expect(stubs.class3.getCall(0).args[0]).to.deep.equal({name: 345});

						expect(stubs.class4.getCall(0).args[0]).to.deep.equal({name: 456});

						expect(stubs.class5.getCall(0).args[0]).to.deep.equal({
							class2Id: 234
						});

						expect(stubs.class5.getCall(1).args[0]).to.deep.equal({
							class4Id: 456
						});
