...
function buildProperties(properties, property, field) {
	...
	if (field.onCreate) {
		properties.onCreate = actionExtend(
			field.onCreate,
			property,
			properties.onCreate
		);
	...
	if (field.onUpdate) {
		properties.onUpdate = actionExtend(
			field.onUpdate,
			property,
			properties.onUpdate
		);
	...
	if (field.key) {
		if (properties.key) {
			throw new Error(
				`bmoor-data.Model does not support compound keys: (${properties.key}, ${property})`
			);
		}
...
class Model {
	...
	getIndex(query) {
		return this.properties.index.reduce((agg, field) => {
	...
	getQuery(query) {
		return this.properties.query.reduce((agg, field) => {
			if (field in query) {
				agg[field] = query[field];
			}
	...
	cleanDelta(delta, type = 'update') {
		return this.properties[type].reduce((agg, field) => {
			if (field in delta) {
	...
	getChanges(datum, delta) {
		...
		return this.properties.update.reduce((agg, field) => {
	...
	getChangeType(delta) {
		const changes = Object.keys(this.cleanDelta(delta)).reduce(
			(agg, property) => {
